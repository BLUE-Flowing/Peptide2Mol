...
def parse_sdf_file(path):
    ...
    ret_data = {
            'element': torch.tensor(data['element'], dtype=torch.int64),
            'pos': torch.tensor(data['pos'], dtype=torch.float32),
            'atom_feature': torch.tensor(data['atom_feature'], dtype=torch.float32),
            'bond_index': torch.tensor(data['bond_index'], dtype=torch.int64),
            'bond_type': torch.tensor(data['bond_type'], dtype=torch.int64),
            'center_of_mass': torch.tensor(data['center_of_mass'], dtype=torch.float32),
        }
    return ret_data, data


def parse_sdf_file_remove(path, to_be_removed):
    # to_be_removed is a list, including the atoms to be removed in the output
    ...
    # Remove atoms and update the bonds
    to_be_removed = set(to_be_removed)
    
    # Filter out the atoms to be removed
    mask = np.array([i not in to_be_removed for i in range(num_atoms)], dtype=bool)
    
    element = element[mask]
    pos = pos[mask]
    feat_mat = feat_mat[mask, :]
    
    # Create a map from old atom indices to new ones
    atom_map = {old_idx: new_idx for new_idx, old_idx in enumerate(np.where(mask)[0])}

    # Filter out bonds involving removed atoms and remap the remaining ones
    valid_bonds = np.array([start not in to_be_removed and end not in to_be_removed 
                            for start, end in zip(edge_index[0], edge_index[1])])
    
    edge_index = edge_index[:, valid_bonds]
    edge_type = edge_type[valid_bonds]

    # Update the edge index with the new atom indices
    edge_index[0] = np.array([atom_map[old_idx] for old_idx in edge_index[0]])
    edge_index[1] = np.array([atom_map[old_idx] for old_idx in edge_index[1]])
