...
def pdb_to_pocket_data(pdb_path, ref_ligand):
    warnings.simplefilter('ignore', BiopythonWarning)
    ptable = Chem.GetPeriodicTable()
    parser = PDBParser()
    
    # Read the reference ligand and compute center and bounding box size
    ligand_data_for_build, ligand_data = parse_sdf_file(ref_ligand)
    ligand_coords = ligand_data['pos']
    center = torch.FloatTensor(ligand_coords.mean(axis=0))
    ligand_min = ligand_coords.min(axis=0)
    ligand_max = ligand_coords.max(axis=0)
    ligand_size = ligand_max - ligand_min
    bbox_size = np.max(ligand_size) + 5.0  # Adding a margin of 5.0 Å
    ...
    # print(ligand_data_for_build)
    data = ProteinLigandData.from_protein_ligand_dicts(
        protein_dict=protein_dict,
        ligand_dict=ligand_data_for_build
    )
    return data
...
if __name__ == '__main__':
    ...
    parser.add_argument('--ligand_path', type=str, default='/home/chengxi/data/work/test/comp2404/PMT/PMT_0718/Pocket2Mol/example/1eeo_PTN1_poc_78.sdf', 
                        help='reference ligand path')
    parser.add_argument('--config', type=str, default='./configs/sample_for_pdb.yml')
    parser.add_argument('--device', type=str, default='cuda')
    parser.add_argument('--outdir', type=str, default='./outputs_conflict_0h_1113')
    ...
    masking = LigandFixMask(args.to_be_removed)
    transform = Compose([
        RefineData(),
    ...
    # # Data
    data_init = pdb_to_pocket_data(args.pdb_path, args.ligand_path)
    data = transform(data_init)
    ...
    init_data_list = get_next(data.to(args.device),   # sample the initial atoms
            model = model,
    ...
    try:
        while len(pool.finished) < config.sample.num_samples:
            ...
            n_tmp = len(queue_tmp)
            next_idx = np.random.choice(np.arange(n_tmp), size=min(config.sample.beam_size, n_tmp), replace=False)
