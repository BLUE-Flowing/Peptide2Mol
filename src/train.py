...
from notebooks.deal_with_mol_5A import Drug3DData
...
def train(cfg: DictConfig) -> Tuple[Dict[str, Any], Dict[str, Any]]:
    ...
    trainer: Trainer = hydra.utils.instantiate(cfg.trainer, callbacks=callbacks, logger=logger, precision="32-true")
    object_dict = {
        "cfg": cfg,
    ...
    if cfg.get("train"):
        ...
        if cfg.get("ckpt_path"):
            ...
            if not fs.exists(ckpt_path):
                ...
                ckpt_path = ckpt_path
                log.info(f'Resuming training from checkpoint {str(ckpt_path)}!')
        else:
            ckpt_path = None
