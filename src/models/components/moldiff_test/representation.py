...
class MolDiff(Module):
    ...
    def get_loss(self, node_type, node_pos, batch_node,
        ...
        # print(node_pert[0].shape, node_pert[0], node_pert[1].shape, node_pert[1], node_pert[2].shape, node_pert[2])
        # for i in node_pert[0]:
        #     print(i)
        # assert False

        # 这里扰动的并不是所有已知的边，而是扰动所有的原子对！ 对于原来有边的，扰动边，对于没有边，是可能给加上边的！
        # 但是对于我们这样的大范围case，这样扰动会产生太大的数据量，不利于模型收敛，我们只要扰动涉及到diffu_idx的边即可
        # print(diff_bond_type_idx,'diff_bond_type_idx',diffu_idx,'diffu_idx')
    ...
    def sample(self, n_graphs, batch_node, halfedge_index, 
        ...
        # print(ref_data.pos_mask, ref_data.pos_padding, 'ref_data.pos_mask, ref_data.pos_padding')
        # print(pos_init1, 'pos_init1')
        # print(ref_data.bond_mask, ref_data.half_edge_padding, 'ref_data.bond_mask, ref_data.halfedge_padding')
        # exit()
        ...
        write_xyz_file(pos_init, element_pred, f'{traj_fn}')
