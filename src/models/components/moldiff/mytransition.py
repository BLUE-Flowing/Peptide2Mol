...
class ContigousTransition(nn.Module):
    ...
    def add_noise(self, x, time_step, batch, diffu_idx):
        ...
        if diffu_idx is not None:
            # print(diffu_idx, diffu_idx.shape)
            # print(pert, pert.shape,'pert')
            # print(x,x.shape,'x')
            pert = torch.where(diffu_idx, x, pert)
            # print(pert, 'pert after')
    ...
    def get_prev_from_recon(self, x_t, x_recon, t, batch):
        # print(x_t, x_recon, t, 'x_t, x_recon, t')
        # exit()
