...
class MolWrapper(nn.Module):
    ...
    def sample(self, batch):
        ...
        while len(pool.finished) < self.num_mols:
            ...
            if not os.path.exists(os.path.join(sdf_dir, f'{name_use}_origin.pdb')):
                try:
                    output_list = self.seperate_outputs_origin(outputs, n_graphs, batch_node, halfedge_index, batch_halfedge)
    ...
    def make_mydata_placeholder(self, n_graphs, ref_data, device=None, max_size=None):
        ...
        ref_data.pocket_or_not = torch.cat([torch.zeros(n_nodes), ref_data.pocket_or_not]*n_graphs, dim=0)
    ...
    def seperate_outputs(self, outputs, n_graphs, batch_node, halfedge_index, batch_halfedge):
        ...
        for i_mol in range(n_graphs):
            ...
            new_pred_this = [outputs_pred[0][ind_node[:outputs_pred[0].shape[0]]],  # node type
    ...
    def seperate_outputs_origin(self, outputs, n_graphs, batch_node, halfedge_index, batch_halfedge):
